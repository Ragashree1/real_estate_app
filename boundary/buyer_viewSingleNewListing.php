...
function shortlist()
{
    ...
    $buyerShortlistController->shortlist($loggedInUsername, $listing_id);
...
function removeShortlist()
{
    ...
    $shortlisted = $buyerRemoveShortlistController->removeShortlist($loggedInUsername, $listing_id);
...
if (empty($singleListing)) {
    ...
    <div class="single-listing" style="position: relative;">
        ...
        <?php if ($shortlisted) : ?>
            ...
            <a href="buyer_viewSingleNewListing.php?listing_id=<?php echo $listing_id . '&' . 'unlike=true' ?>" class="btn btn-success like-button btn-lg" style="position: absolute; top: 10px; right: 50px;" >
                <i class="fas fa-check"></i> shortlisted
            </a>
        <?php else : ?>
            <!-- If the item is not shortlisted -->
            <a href="buyer_viewSingleNewListing.php?listing_id=<?php echo $listing_id . '&' . 'like=true' ?>" class="btn btn-outline-primary like-button btn-lg" style="position: absolute; top: 10px; right: 50px;">
